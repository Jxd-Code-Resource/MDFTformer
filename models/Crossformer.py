...
class Crossformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size=4,
        ...
        # 防止数据长度无法被seg_len整除
        self.pad_in_len = ceil(1.0 * in_len / seg_len) * seg_len  # ceil向上取整
        self.pad_out_len = ceil(1.0 * out_len / seg_len) * seg_len
        self.in_len_add = self.pad_in_len - self.in_len  # 计算了输入序列长度的增加量
    ...
    def forward(self, x_seq):
        ...
        # 如果填充长度不为0，就在输入序列的开头添加了一部分重复的信息，以满足填充后序列长度的需求
        if (self.in_len_add != 0):
            x_seq = torch.cat((x_seq[:, :1, :].expand(-1, self.in_len_add, -1), x_seq), dim=1)

        x_seq = self.enc_value_embedding(x_seq)# 将原始数据送进一个线性层，进行线性变换
